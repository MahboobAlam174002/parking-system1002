...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateSlots(vehicleType) {
        ...
        slots.forEach(slot => {
            ...
            button.addEventListener('click', function () {
                ...
                slotForm.dataset.selectedSlot = slot.slotNumber; // Store selected slot
            });
    ...
    slotForm.addEventListener("submit", function (e) {
        ...
        const slotNumber = parseInt(slotForm.dataset.selectedSlot); // Get selected slot
        const passKey = passKeyInput.value;

        let slot;
        if (vehicleType === 'car') {
            slot = carSlots.find(s => s.slotNumber === slotNumber);
        } else {
            slot = bikeSlots.find(s => s.slotNumber === slotNumber);
        }
